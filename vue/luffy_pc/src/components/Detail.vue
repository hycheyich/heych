      ...
      <div class="course-info">
        ...
        <div class="wrap-right">
          ...
          <div class="sale-time" v-if="course.discount_name">
            ...
            <p class="expire">距离结束：仅剩 {{parseInt( course.has_time/(24*60*60) ) | time_format}}天
              {{parseInt(course.has_time/3600%24) | time_format}}小时 {{parseInt(course.has_time/60%60) | time_format}}分
              <span class="second">{{parseInt(course.has_time%60) | time_format}}</span> 秒</p>
          </div>
          <p class="course-price" v-if="course.has_time>0">
            <span>活动价</span>
            <span class="discount">¥{{course.discount_price.toFixed(2)}}</span>
          ...
          <div class="buy">
            ...
            <div class="add-cart"><img src="/static/image/cart-yellow.svg" alt=""><span @click="add_cart">加入购物车</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="course-tab-list">
          ...
          <div class="tab-item" v-if="tabIndex==2">
            <div class="tab-item-title">
              ...
              <p class="chapter-length">共{{course_chapter.length}}章 147个课时</p>
            </div>
            <div class="chapter-item" v-for="(coursechapter,k) in course_chapter" :key="k">
              <p class="chapter-title"><img src="/static/img/1.svg" alt="">第{{coursechapter.chapter}}章·{{coursechapter.name}}
              </p>
              <ul class="lesson-list">
                <li class="lesson-item" :key="kk" v-for="(lesson,kk)  in coursechapter.lessons">
                  <p class="name"><span class="index">{{coursechapter.chapter}}-{{lesson.id}}</span>{{lesson.name}}<span
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$axios.get(`${this.$settings.Host}/courses/${this.course_id}/`).then(response => {
          ...
          let t = setInterval(() => {
            if (this.course.has_time > 1) {
              ...
              this.course.has_time = 0;
              this.course.discount_name = "";
              this.course.discount_price = -1;
              clearInterval(t);
            }
          },1000)
        }).catch(error => {
      ...
      get_course_chapter() {
        // 获取课程章节
        this.$axios.get(`${this.$settings.Host}/courses/chapter/`, {
          params: this.course_id
        }).then(response => {
          console.log(response.data)
          this.course_chapter = response.data;
        }).catch(error => {
          this.$message.error('获取课程章节失败')
        })
      }, add_cart() {
        // 添加商品课程到购物车中
        // 0. 先判断用户是否登录
        let user_token = this.$settings.token();
        if (!user_token) {
          let self = this;
        ...
        this.$axios.post(`${this.$settings.Host}/cart/`,
          ...
          if (error.response.status == 400) {//没有登陆或者登陆超时
            let self = this;
            this.$confirm('对不起，您尚未登录，请登录后继续购买, 是否继续?', '路飞学城', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
            ...
            this.$message.error('购物车添加商品失败，联系客服工作人员')
          }
        })
